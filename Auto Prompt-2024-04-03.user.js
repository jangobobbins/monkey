...
(function() {
    ...
    async function prompt(sex, character, generation) {
        return new Promise((resolve, reject) => {
            let textbox = document.querySelector('#prompt-textarea');
            if (!textbox) {
    ...
    async function replay() {
        return new Promise((resolve, reject) => {
            let list = document.getElementsByClassName("p-1 rounded-md text-token-text-tertiary hover:text-token-text-primary md:invisible md:group-hover:visible md:group-[.final-completion]:visible");
            console.log("Replay");
    ...
    startBtn.onclick = startGenerating;
